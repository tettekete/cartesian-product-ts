...
import {cartesianProductWithSizeList} from '../src/lib/cartesianProduct';

describe('cartesianProductWithSizeList', () =>
{
	test('enumerates every index combination for [2, 3]', () =>
	{
		const combinations = Array.from(cartesianProductWithSizeList([2, 3]));
		...
		const combinations = Array.from(cartesianProductWithSizeList(sizeList));
